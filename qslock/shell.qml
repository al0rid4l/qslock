...
Scope {
	...
	final property bool bgImgInAnimationRunning: false
	final property bool showLoading: false
	final property bool bgImgTimeout: false
	final property bool focusPasswordInput: false
	final property bool disablePasswordInput: false
	final property color imgLoadErrorBgColor: "transparent"
	...
	Timer {
		...
		onTriggered: () => !root.bgImgInAnimationRunning && (root.imgLoadErrorBgColor = "#ffffff", root.bgImgTimeout = true)
	}
	...
	Timer {
		...
		onTriggered: () => {
			if (!auth.active && !root.showLoading) {
				auth.start();
			}
		}
	}
	...
	IdleMonitor {
		...
		onIsIdleChanged: () => autoBlur.isIdle && (root.focusPasswordInput = false);
	}
	...
	Timer {
		...
		// TODO
		onTriggered: () => hintMsg.clear() && (root.disablePasswordInput = false);
	}
	...
	IpcHandler {
		...
		function unlock(token: string): bool {
			let resultl = ipcHdr.compare(token, root.token);
			if (result) {
	...
	Process {
		...
		stdout: StdioCollector {
			onStreamFinished: function() {
				...
				let file = '/run/user/' + root.uid + '/qslocktoken';
				let cmd = "echo '" + root.token + "' >" + file + " && chmod 600 " + file;
	...
	WlSessionLock {
		...
		WlSessionLockSurface {
			...
			onVisibleChanged: () => {
				// 亮屏触发howdy
				if (!auth.active && !auth.firstStart && screen.visible && !root.disablePasswordInput && !root.bgImgInAnimationRunning) {
					auth.start();
				}
			}

			Component.onCompleted: () => {
				if (!uidFetcher.running) uidFetcher.running = true;
			}
			...
			Shortcut {
				...
				onActivated: e => {
					if (!auth.active && !root.bgImgInAnimationRunning) {
			...
			Rectangle {
				id: lockMask
				color: root.imgLoadErrorBgColor
				anchors.fill: parent
				clip: true
				...
				Rectangle {
					...
					NumberAnimation {
						...
						duration: 600
						onStopped: () => QSLockConfig.autoFocus && (root.focusPasswordInput = true)
					}
					...
					NumberAnimation {
						...
						duration: 300
						onStarted: () => root.bgImgInAnimationRunning = false
						onStopped: () => {
							screenlock.locked = false;
							root.bgImgInAnimationRunning = false;
							Qt.quit();
						}
					}
					...
					Image {
						...
						onStatusChanged: () => {
							if (QSLockConfig.inAnimationEnabled && !root.bgImgTimeout && bgImg.status === Image.Ready) {
								bgImgIn.start();
							} else if (bgImg.status === Image.Error) {
								lockwrapper.y = 0;
								root.imgLoadErrorBgColor = "#ffffff"
					...
					Connections {
						target: root
						// TODO 应当只设置active屏幕的passwordInput
						function onFocusPasswordInputChanged() {
							passwordInput.setFocus(root.focusPasswordInput);
						}
						function onDisablePasswordInputChanged() {
							passwordInput.disable = root.disablePasswordInput;
						}
					}
					...
					function submit(text: string) {
						if (text.length === 0) {
							...
							return;
						} else if (root.disablePasswordInput || root.bgImgInAnimationRunning) {
							return;
						} else {
							passwordInput.error = false;
							root.disablePasswordInput = true;
		...
		onCompleted: result => {
			...
			switch (result) {
				case PamResult.Success:
					root.disablePasswordInput = false;
